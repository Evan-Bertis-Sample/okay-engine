...
static void __gameInitialize() {
    ...
    okay::OkayTextOptions textOpt{
        .font = assetManager
                    ->loadEngineAssetSync<okay::OkayFontManager::FontHandle>("fonts/ARIAL.TTF",
                                                                             fontLoadOptions)
                    .value()
                    .asset,
    ...
    okay::OkayMesh cube = renderer->meshBuffer().addMesh(
        okay::primitives::box()
            .sizeSet({0.3f, 0.3f, 0.3f})
            .build(
                okay::primitives::colorTransform<okay::primitives::BoxBuilder>(glm::vec4(1.0f))));
    ...
    okay::OkayMaterialHandle sunMat =
        renderer->materialRegistry().registerMaterial(shader, std::move(materialProprties));
    g_sun = renderer->world().addRenderEntity(
        okay::OkayTransform({0.0f, 0.0f, 0.0f}, {0.05f, 0.05f, 0.05f}), sunMat, textMesh);
    ...
    albedo = texture;
    okay::OkayMaterialHandle planetMat =
        renderer->materialRegistry().registerMaterial(shader, std::move(materialProprties));
    g_planet = renderer->world().addRenderEntity(
        okay::OkayTransform({-40.0f, 0.0f, 0.0f}, {0.3f, 0.3f, 0.3f}), planetMat, teapot);

    // moon material
    materialProprties = std::make_unique<okay::LitMaterial>();
    materialProprties->color.set(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
    materialProprties->albedo = texture;
    okay::OkayMaterialHandle moonMat =
        renderer->materialRegistry().registerMaterial(shader, std::move(materialProprties));
    g_moon = renderer->world().addRenderEntity(
        okay::OkayTransform({30.0f, 0.0f, 0.0f}, {0.2f, 0.2f, 0.2f}), moonMat, teapot);

    g_debugSphere = renderer->world().addRenderEntity(
        okay::OkayTransform({0.0f, 0.0f, 0.0f}, {0.005f, 0.005f, 0.005f}), moonMat, teapot);
    ...
    for (int i = 0; i < numCubes; ++i) {
        ...
        renderer->world().addRenderEntity(
            okay::OkayTransform(pos, scale, glm::quat()), sunMat, cube);
    }

    // add a directional light
    renderer->world().addLight(okay::OkayLight::directional(
        glm::vec3(-0.5f, -1, 0.5f), glm::vec3(1.0, 0.8745f, 0.1333f), 0.6f));

    // add a blue point light
    g_pointLight = renderer->world().addLight(
        okay::OkayLight::point(glm::vec3(0, 0, 0), 3.0f, glm::vec3(0.0f, 0.0f, 1.0f), 20.0f));
}
...
static void __gameUpdate() {
    ...
    // Rotate the sun
    g_sun->transform.rotation =
        glm::angleAxis(glm::radians(45.0f) * okay::Engine.time->timeSinceStartSec() * 1.0f,
                       glm::vec3(0.0f, 1.0f, 0.0f));

    // Rotate the planet
    g_planet->transform.rotation =
        glm::angleAxis(glm::radians(45.0f) * okay::Engine.time->timeSinceStartSec() * 1.5f,
                       glm::vec3(0.0f, 1.0f, 0.0f));

    // Rotate the moon
    g_moon->transform.rotation =
        glm::angleAxis(glm::radians(45.0f) * okay::Engine.time->timeSinceStartSec() * 2.0f,
                       glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    glm::vec3 lightPos = glm::vec3(std::sin(angle) * r, 0.0f, std::cos(angle) * r);

    renderer->world().getLight(g_pointLight).setPosition(lightPos);
    renderer->world().getLight(g_pointLight).color =
        glm::vec4(abs(sin(angle * 3.0f)), 0.0f, 1.0f, 1.0f);
