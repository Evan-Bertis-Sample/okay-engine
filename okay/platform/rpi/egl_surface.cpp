...
EGLConfig choose_config(EGLDisplay dpy, bool requestMSAA) {
...
void Surface::initialize() {
    ...
    _impl->dpy = getPlatformDisplay
                     ...
                     : eglGetDisplay(reinterpret_cast<EGLNativeDisplayType>(_impl->gbmDev));
    ...
    _impl->cfgEGL = choose_config(_impl->dpy, true);
    if (!_impl->cfgEGL) {
        throw std::runtime_error("eglChooseConfig failed to find a depth-capable config");
    }
    _impl->surf = eglCreateWindowSurface(
        _impl->dpy, _impl->cfgEGL, reinterpret_cast<EGLNativeWindowType>(_impl->gbmSurf), nullptr);
    ...
    _impl->ctx = eglCreateContext(_impl->dpy, _impl->cfgEGL, EGL_NO_CONTEXT, ctxAttribs);
    ...
    if (!gladLoadGL()) {
        std::cout << "Failed to initialize GLAD for GLES" << std::endl;
    }
...
void Surface::swapBuffers() {
    if (!eglSwapBuffers(_impl->dpy, _impl->surf)) {
...
void Surface::destroy() {
    ...
    if (_impl->dpy != EGL_NO_DISPLAY) {
        ...
        if (_impl->surf != EGL_NO_SURFACE) {
            ...
            _impl->surf = EGL_NO_SURFACE;
        }

        if (_impl->ctx != EGL_NO_CONTEXT) {
            eglDestroyContext(_impl->dpy, _impl->ctx);
            _impl->ctx = EGL_NO_CONTEXT;
        }
